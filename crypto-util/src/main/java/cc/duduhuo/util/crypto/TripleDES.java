...
public class TripleDES {
    ...
    private abstract static class Base<T extends Base<T>> implements CryptoProtocol {
        ...
        /**
         * 密钥长度（字节数，可取值 16,24）。默认 -1, 表示根据传入的密钥长度而定，自动填充到最接近的长度。 例如，传入 16 字节密钥，则刚好到 16 字节，不填充；传入 17 字节密钥，则填充到 24 字节。
         * 如果指定长度，当传入的密钥不足长度时，则填充，超出长度时，则截断。
         * 当 keyLength = -1 时，如果传入密钥长度超过 24 字节，则截断。
         */
        ...
        /**
         * 设置密钥长度（字节数，可取值 16,24）。默认 -1, 表示根据传入的密钥长度而定，自动填充到最接近的长度。
         */
        ...
        public SecretKey createKey(byte[] key) {
            return Utils.createTripleDesKey(key, keyLength, keyPadding);
        }
