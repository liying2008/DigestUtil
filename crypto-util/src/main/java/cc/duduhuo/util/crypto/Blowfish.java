...
public class Blowfish {
    ...
    private abstract static class Base<T extends Base<T>> implements CryptoProtocol {
        ...
        /**
         * 密钥长度（字节数，可取值 4-56）。默认 -1, 表示根据传入的密钥长度而定，最小4字节，最大56字节，不足4字节则填充，超过56字节则截断。
        ...
        /**
         * 设置密钥长度（字节数，可取值 4-56）。默认 -1, 表示根据传入的密钥长度而定，最小4字节，最大56字节，不足4字节则填充，超过56字节则截断。
         */
        ...
        public SecretKey createKey(byte[] key) {
            return Utils.createBlowfishKey(key, keyLength, keyPadding);
        }
